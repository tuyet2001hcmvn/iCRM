...
namespace Customer.Controllers
{
    public class CompetitorController : BaseController
    {
        ...
        public ActionResult _List(Guid? id, bool? isLoadContent = false)
        {
            return ExecuteSearch(() =>
            {
                var result = (from p in _context.ProfileModel
                              ...
                              where a.ProfileId == id && a.PartnerType == ConstPartnerType.DoiThu //Đối thủ
        ...
        public ActionResult Save(Guid? ProfileId, Guid? PartnerId)
        {
            ...
            {
                //var existsPartner = _context.Profile_Opportunity_PartnerModel.Where(p => p.ProfileId == ProfileId && p.PartnerType == ConstPartnerType.DoiThu).FirstOrDefault();
                #region Create
                ...
                partner.PartnerType = ConstPartnerType.DoiThu; //Đối thủ
                ...
                return Json(new
                {
                    ...
                    Success = true,
                    Data = string.Format(LanguageResource.Alert_Create_Success, LanguageResource.TabCompetitor.ToLower()),
        ...
        public ActionResult SetMain(Guid? OpportunityPartnerId)
        {
            return ExecuteContainer(() =>
            {
                ...
                if (partner != null)
                {
                    ...
                    var remainPartner = _context.Profile_Opportunity_PartnerModel.Where(p => p.ProfileId == partner.ProfileId && p.OpportunityPartnerId != OpportunityPartnerId && p.PartnerType == ConstPartnerType.DoiThu).ToList();
                    ...
                    return Json(new
                    {
                        Code = System.Net.HttpStatusCode.Created,
                        Success = true,
                        Data = string.Format(LanguageResource.Alert_Edit_Success, LanguageResource.TabCompetitor.ToLower()),
        ...
        public ActionResult Delete(Guid id)
        {
            ...
            {
                var competitor = _context.Profile_Opportunity_PartnerModel.FirstOrDefault(p => p.OpportunityPartnerId == id);
                if (competitor != null)
                {
                    var ProfileId = competitor.ProfileId;
                    _context.Entry(competitor).State = EntityState.Deleted;
                    _context.SaveChanges();

                    //var remainCompetitor = _context.Profile_Opportunity_PartnerModel.Where(p => p.ProfileId == ProfileId && p.PartnerType == ConstPartnerType.DoiThu).ToList();
                    ...
                    return Json(new
                    {
                        Code = System.Net.HttpStatusCode.Created,
                        Success = true,
                        Data = string.Format(LanguageResource.Alert_Delete_Success, LanguageResource.TabCompetitor.ToLower())
                    });
                }
                else
                {
