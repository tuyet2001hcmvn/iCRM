...
namespace Reports.Controllers
{
    public class TaskTicketMLCReportController : BaseController
    {
        ...
        private List<SearchDisplayModel> GetSearchInfoToDisplay(TaskTicketMLCReportSearchModel searchViewModel)
        {
            ...
            //if (!string.IsNullOrEmpty(searchViewModel.DepartmentCode))
            //{
            //    SearchDisplayModel filter = new SearchDisplayModel();
            //    filter.DisplayName = "Phòng ban";
            //    var dep = _context.DepartmentModel.FirstOrDefault(s => s.DepartmentCode == searchViewModel.DepartmentCode);
            ...
            if (!string.IsNullOrEmpty(searchViewModel.CreateBy))
            {
                SearchDisplayModel filter = new SearchDisplayModel();
                filter.DisplayName = "Người tạo";
                var sale = _context.SalesEmployeeModel.FirstOrDefault(s => s.SalesEmployeeCode == searchViewModel.CreateBy);
                var value = sale.SalesEmployeeCode + " | " + sale.SalesEmployeeName;
                filter.DisplayValue = value;
                filterList.Add(filter);
            }
