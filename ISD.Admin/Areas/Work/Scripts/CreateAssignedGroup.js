...
function SaveData(data, isToEditMode) {
    loading2();
    $.ajax({
        type: "POST",
        url: "/Work/AssignedGroup/Create",
        data: data,
        dataType: "json",
        success: function (res) {
            if (res.IsSuccess === true) {
                ...
                if (isToEditMode) {
                    employeeList = [];
                    window.location.href = "/Work/AssignedGroup/Edit/" + res.Data + "?Type=" + res.Type;
                    
                }
                else {
                    employeeList = [];
                    window.location.href = "/Work/AssignedGroup/Index/?Type=" + res.Type;
                }
            }
            else {
